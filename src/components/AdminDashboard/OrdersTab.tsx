...
export default function OrdersTab({ adminDict, lang }: OrdersTabProps) {
  ...
  const getStatusText = (status: OrderStatus) => {
    return od.status[status.toLowerCase() as keyof typeof od.status] || status;
  };
      ...
      {isModalOpen && selectedOrder && (
        <ModalOverlay onClick={handleCloseModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 bg-white p-6 rounded-lg shadow">
              <div>
                ...
                <p>
                  <strong>{od.customerLabel}</strong> {getCustomerName(selectedOrder)}
                </p>
                {selectedOrder.user?.email && (
                  <p>
                    <strong>{od.customerEmailLabel}</strong> {selectedOrder.user.email}
                  </p>
                )}
                {!selectedOrder.user && selectedOrder.guestCheckoutInfo?.email && (
                  <p>
                    <strong>{od.customerEmailLabel}</strong> {selectedOrder.guestCheckoutInfo.email}
                  </p>
                )}
                <p>
                  <strong>{od.orderDateLabel}</strong>{' '}
                  {new Date(selectedOrder.createdAt).toLocaleDateString()}
                </p>
              </div>
              <div>
                <p>
                  <strong>{od.orderStatusLabel}</strong>{' '}
                  <span
                    className={`px-2 py-1 rounded-full text-sm ${selectedOrder.status === OrderStatus.Delivered ? 'bg-green-200 text-green-800' : 'bg-yellow-200 text-yellow-800'}`}
                  >
                    {getStatusText(selectedOrder.status)}
                  </span>
                </p>
                <p>
                  <strong>{od.totalAmountLabel}</strong> ${selectedOrder.totalAmount.toFixed(2)}
                </p>
                <div className="mt-4">
                  <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold mb-2">{od.shippingAddressLabel}</h3>
              ...
              {selectedOrder.billingAddress && (
                <div className="bg-white p-6 rounded-lg shadow">
                  <h3 className="text-lg font-semibold mb-2">{od.billingAddressLabel}</h3>
            ...
            <div className="mb-6 bg-white p-6 rounded-lg shadow">
              <h3 className="text-xl font-semibold mb-4">{od.itemsTable.title}</h3>
            ...
            {selectedOrder.paymentDetails && (
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-xl font-semibold mb-4">{od.paymentDetails.title}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <p>
                    <strong>{od.paymentDetails.paymentIdLabel}</strong>{' '}
                    {selectedOrder.paymentDetails.paymentId}
                  </p>
                  <p>
                    <strong>{od.paymentDetails.paymentMethodLabel}</strong>{' '}
                    {selectedOrder.paymentDetails.paymentMethod}
                  </p>
                  <p>
                    <strong>{od.paymentDetails.paymentStatusLabel}</strong>{' '}
                    {selectedOrder.paymentDetails.paymentStatus}
                  </p>
                  <p>
                    <strong>{od.paymentDetails.transactionIdLabel}</strong>{' '}
      ...
      {isStatusUpdateModalOpen && selectedOrder && (
        <ModalOverlay onClick={handleCloseStatusUpdateModal}>
          <ModalContent onClick={(e) => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4">{od.updateStatusModal.title}</h3>
            <div className="mt-4">
              <label
            ...
            <div className="items-center px-4 py-3 mt-6 text-right">
              <button
                ...
                className="px-4 py-2 bg-blue-500 text-white text-base font-medium rounded-md shadow-sm hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 disabled:opacity-50"
              >
                {loadingStatusUpdate ? commonD.loading : od.updateStatusModal.updateButton}
              </button>
              <button
                onClick={handleCloseStatusUpdateModal}
                className="ml-3 px-4 py-2 bg-gray-200 text-gray-700 text-base font-medium rounded-md shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-300"
              >
